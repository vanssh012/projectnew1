...
export default function FarewellPage() {
  const [events, setEvents] = useState(() => getFallbackCards("farewell"));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const load = async () => {
      try {
        const data = await fetchEvents("farewell");
        const fallback = getFallbackCards("farewell");
        setEvents(data.length > 0 ? data : fallback);
      } catch {
        setEvents(getFallbackCards("farewell"));
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="page-load-animate">
      ...
      <section style={{ minHeight: "80vh", display: "flex", alignItems: "center", padding: "80px 24px", background: "#000" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", width: "100%" }}>
          <div className="label" style={{ color: "var(--gold)", marginBottom: 24 }}>
            ✦ FAREWELL
          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 40 }}>
              <div className="fade-section">
                ...
                <div style={{ color: "rgba(255,255,255,0.5)" }}>
                  a private space that feels like yours. not a club. not a banquet hall.
                </div>
              </div>
              <div className="fade-section">
